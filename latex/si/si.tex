...
\subsection{Example 2: Sequential Mode Operations}
...
\begin{itemize}
    ...
    \item \inpy{pp.replacement_scan()} replaces a segment of the input sequence with an insert at each specified position. With \inpy{mode='sequential'}, it systematically enumerates all position states. Since \inpy{ins_pool} also has sequential states, the two are combined via Cartesian product: $2 \text{ inserts} \times 4 \text{ positions} = 8$ total states.
    \begin{center}
        \texttt{[PLACEHOLDER: example2\_rep\_states.png showing the 8 replacement states]}
    \end{center}
    
    \item \inpy{pp.deletion_scan()} removes a segment of specified length at each position, replacing it with deletion markers (\inpy{'-'}). With 4 positions, this yields 4 states.
    \begin{center}
        \texttt{[PLACEHOLDER: example2\_del\_states.png showing the 4 deletion states]}
    \end{center}
    ...
    \item \inpy{generate_seqs(num_complete_iterations=1)} iterates through all 12 states exactly once. Unlike \inpy{num_seqs=N} which samples $N$ sequences (potentially with repetition for random operations), \inpy{num_complete_iterations=N} guarantees that every state is visited exactly $N$ times.
    
    \item The resulting sequences all have the same length because \inpy{replacement_scan} replaces 5 nucleotides with a 5-nucleotide insert, and \inpy{deletion_scan} replaces 5 nucleotides with 5 deletion markers.
    \begin{center}
        \texttt{[PLACEHOLDER: example2\_seqs.png showing all 12 output sequences]}
    \end{center}
    
    \item The computation tree for this example shows how the counters are composed:
    \begin{center}
        \texttt{[PLACEHOLDER: example2\_tree.png showing the computation tree with counter states]}
    \end{center}
    
    \item The design card columns in the output dataframe track which state each operation was in when generating each sequence, enabling downstream analysis:
    \begin{center}
        \texttt{[PLACEHOLDER: example2\_df.png showing the design card columns]}
    \end{center}
\end{itemize}
