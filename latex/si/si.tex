...
adapter_3_iupac = 'GGA.NNNNN.CAG'      # 3 + 5N barcode + 3

# Create pool of randomly mutagenized wt_seq sequences
mut_pool = pp.mutagenize(wt_seq, mutation_rate=0.10)

# Create pool of 3' adaptors with random barcodes
adapter_3_pool = pp.from_iupac_motif(adapter_3_iupac)
...
display(df['seq', 'positions', 'wt_chars', 'mut_chars'])
\end{minted}

\noindent\textbf{Example output} (mutations in UPPERCASE, degenerate positions in lowercase):

% \scriptsize
% \begin{minted}{python}
%   seq                                       ... positions      wt_chars      mut_chars
% 0 GAAC.ATCGATCGtTAaCGATCGAa.TGGA.gacgt.CCAG ... (8,11,19)      (A,T,T)       (t,a,a)
% 1 GAAC.ATCGATCGATATCtAcCcAT.TGGA.cctgt.CCAG ... (13,15,17)     (G,T,G)       (t,c,c)
% 2 GAAC.ATCGATCaATAaCcATtGAT.TGGA.acaac.CCAG ... (7,11,13,16)   (G,T,G,C)     (a,a,c,t)
% 3 GAAC.ATCccTCGATATtGATCGAT.TGGA.ctaga.CCAG ... (3,4,12)       (G,A,C)       (c,c,t)
% 4 GAAC.ATCcATCGATATCGcTgGAT.TGGA.agctc.CCAG ... (3,14,16)      (G,A,C)       (c,c,g)
% \end{minted}
% \normalsize

Key points:
\begin{itemize}
    \item \inpy{mode='random'} is the default---random sampling from the combinatorial space
    \item \mintinline{python}{mark_changes=True} highlights mutations in UPPERCASE
    \item \mintinline{python}{mark_degenerate=True} shows randomly-resolved degenerate positions in lowercase
    \item \mintinline{python}{generate_seqs(num_seqs=N)} samples $N$ sequences
    \item Plain strings can be passed directly to \mintinline{python}{join()} and \mintinline{python}{mutagenize()}
\end{itemize}
