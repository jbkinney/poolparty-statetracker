...
### Added
- `from_vcf`: read a VCF and a reference FASTA and create reference and alternate
  windows around variants. By default, it emits one reference window per distinct
  usable site and one window per usable alternate. Design cards carry the VCF
  coordinates, both alleles, `variant_type`, the `ID` and `FILTER` columns, the
  window bounds, and any requested `INFO` fields.

  `REF` is checked against the FASTA case-insensitively; the call fails when more
  than 20% of comparable records disagree. Input that cannot be turned into a
  window is rejected and counted on
