...
how to initialise a pool context, the attributes available on every pool, and
the methods for generating and exporting sequences.
...
Pools must be created inside an active **Party** context. PoolParty provides
two ways to set one up.

``pp.init()`` — persistent context
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Creates (or resets) a long-lived Party that stays active for the rest of the
session. Use this in notebooks and interactive scripts where you want a single
shared context.

.. code-block:: python

    import poolparty as pp
    pp.init()                          # one-time setup

    pool = pp.from_seq("ACGT")         # works anywhere after init()
...
.. list-table::
   ...
   * - ``genetic_code``
     - ``str | dict``
     - ``"standard"``
     - Genetic code for ORF operations. Pass a string identifier (e.g.
       ``"standard"``) or a custom codon dictionary.
   * - ``log_level``
     - ``str | None``
     - ``None``
     - If set, configures logging at the given level (``"DEBUG"``,
       ``"INFO"``, ``"WARNING"``, etc.).

``with pp.Party()`` — scoped context
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Creates a temporary Party that lives only as long as the ``with`` block.
Use this when you need **isolation** — for example, running independent
experiments, writing reusable functions, or testing.
...
.. code-block:: python
    ...
    with pp.Party() as party:
        ...
        df   = pool.generate_library()
    # context closed — calling pp.from_seq() here would use the default party

Contexts nest automatically. The inner context becomes active and the outer is
restored when the inner block exits:
...
.. code-block:: python
    ...
    with pp.Party() as outer:
        ...
        with pp.Party() as inner:
            # inner is now the active party
            other = pp.from_seq("TTTT")
...
.. list-table::
   ...
   * - ``genetic_code``
     - ``str | dict``
     - ``"standard"``
     - Genetic code for ORF operations.
