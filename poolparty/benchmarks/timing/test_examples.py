...
def workload_mpra_example(
    ...
    mutated_pool = (
        template_pool.stylize(region="cre", style="goldenrod")
        .mutagenize(
            region="cre",
            num_mutations=2,
    ...
    deletion_pool = (
        template_pool.stylize(region="cre", style="salmon")
        .deletion_scan(
            region="cre",
            deletion_length=6,
    ...
    insertion_pool = (
        template_pool.stylize(region="cre", style="blue")
        .insertion_scan(
            region="cre",
            ins_pool=sites_pool,
            positions=slice(None, None, 5),
        ...
        .named("insertion_pool")
    )

    shuffle_pool = template_pool.stylize(region="cre", style="purple").shuffle_scan(
        region="cre",
        shuffle_length=6,
    ...
    combo_pool = (
        pp.stack([mutated_pool, recomb_pool, deletion_pool, insertion_pool, shuffle_pool])
        .named("stack_pool")
