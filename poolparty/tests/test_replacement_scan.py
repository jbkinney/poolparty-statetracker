...
from poolparty.operations.scan import replacement_scan
...
class TestReplacementScanMarkChanges:
    """Test mark_changes parameter."""
    
    def test_mark_changes_true_swaps_case(self):
        """Test that mark_changes=True swaps insert case."""
        with pp.Party() as party:
            bg = pp.from_seqs(['AAAAAAAAAA'])
            ins = pp.from_seqs(['TTT'])
    ...
    def test_mark_changes_false_preserves_case(self):
        """Test that mark_changes=False preserves insert case."""
        with pp.Party() as party:
            bg = pp.from_seqs(['AAAAAAAAAA'])
            ins = pp.from_seqs(['TTT'])
            result = replacement_scan(bg, ins, positions=[0], 
    ...
    def test_mark_changes_none_uses_party_default_false(self):
        """Test that mark_changes=None uses party default (False by default)."""
        with pp.Party() as party:
            bg = pp.from_seqs(['AAAAAAAAAA'])
            ins = pp.from_seqs(['TTT'])
            result = replacement_scan(bg, ins, positions=[0], mode='sequential').named('result')
    ...
    def test_mark_changes_none_uses_party_default_true(self):
        """Test that mark_changes=None uses party default when set to True."""
        with pp.Party() as party:
    ...
    def test_mark_changes_with_lowercase_insert(self):
        """Test mark_changes with already lowercase insert."""
        with pp.Party() as party:
            bg = pp.from_seqs(['AAAAAAAAAA'])
            ins = pp.from_seqs(['ttt'])
            result = replacement_scan(bg, ins, positions=[0], 
    ...
    def test_mark_changes_at_end_position(self):
        """Test mark_changes at the end position."""
        with pp.Party() as party:
            bg = pp.from_seqs(['AAAAAAAAAA'])
            ins = pp.from_seqs(['TTT'])
