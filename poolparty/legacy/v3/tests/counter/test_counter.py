...
import pytest
from poolparty.counter import Counter, CounterManager, synchronize_counters, multiply_counters, sum_counters
...
class TestStateManagement:
    ...
    def test_state_wraps_on_set(self):
        """State wraps around when set beyond num_states."""
        A = Counter(num_states=3, name='A')
        A.state = 5  # 5 % 3 = 2
        assert A.state == 2
...
class TestComplexGraphs:
    ...
    def test_chained_products(self):
        ...
        # Set ABC to 7: decompose as (ABC_state) = C_state * 6 + AB_state
        # 7 = 1 * 6 + 1, so C=1, AB=1
        # AB=1: 1 = B_state * 2 + A_state = 0 * 2 + 1, so B=0, A=1
        ABC.state = 7
        assert C.state == 1
        assert AB.state == 1
        assert A.state == 1
...
class TestInactiveState:
    """Test inactive state (-1) behavior."""
    
    def test_sum_sets_inactive_branch(self):
        """Sum decomposition sets inactive branch to -1."""
        A = Counter(num_states=2, name='A')
        B = Counter(num_states=3, name='B')
        C = A + B
    ...
    def test_inactive_method(self):
        """inactive() method sets state to -1."""
        A = Counter(num_states=3, name='A')
        A.state = 2
        A.inactive()
        assert A.state == -1
    ...
    def test_sum_iteration_with_inactive(self):
        """Iterate sum shows -1 for inactive branch."""
        A = Counter(num_states=2, name='A')
        ...
        expected = [
            (0, -1, True, False),   # A active
            (1, -1, True, False),   # A active
            (-1, 0, False, True),   # B active
            (-1, 1, False, True),   # B active
        ]
...
class TestCopy:
    ...
    def test_copy_composite_counter_shares_parents(self):
        ...
        # D shares parents with C
        assert D._parents is C._parents
        assert D._parents[0] is A
        assert D._parents[1] is B
    ...
    def test_copy_preserves_inactive_state(self):
        """Copy preserves inactive state (-1)."""
        A = Counter(num_states=5, name='A')
        A.state = -1
        B = A.copy()
        assert B.state == -1
...
class TestDeepcopy:
    ...
    def test_deepcopy_preserves_inactive_state(self):
        """Deepcopy preserves inactive state (-1)."""
        A = Counter(num_states=5, name='A')
        A.state = -1
        B = A.deepcopy()
        assert B.state == -1
