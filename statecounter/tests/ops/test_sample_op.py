...
from statecounter import Counter, Manager, SampleOp, sample, product
...
class TestSampleOperation:
    ...
    def test_sample_with_num_states(self):
        ...
        with Manager():
            ...
            B = sample(A, num_states=5, seed=42)
    ...
    def test_sample_seed_reproducibility(self):
        ...
        with Manager():
            A1 = Counter(num_states=20, name='A1')
            B1 = sample(A1, num_states=5, seed=123)
            
            A2 = Counter(num_states=20, name='A2')
            B2 = sample(A2, num_states=5, seed=123)
    ...
    def test_sample_different_seeds_different_results(self):
        ...
        with Manager():
            A1 = Counter(num_states=20, name='A1')
            B1 = sample(A1, num_states=5, seed=1)
            
            A2 = Counter(num_states=20, name='A2')
            B2 = sample(A2, num_states=5, seed=2)
    ...
    def test_sample_with_replacement_true(self):
        ...
        with Manager():
            ...
            B = sample(A, num_states=10, seed=42, with_replacement=True)
            assert B.num_states == 10
    ...
    def test_sample_with_replacement_false_valid(self):
        ...
        with Manager():
            A = Counter(num_states=10, name='A')
            B = sample(A, num_states=5, seed=42, with_replacement=False)
            assert B.num_states == 5
    ...
    def test_sample_with_replacement_false_exceeds_raises(self):
        """With with_replacement=False, exceeding parent.num_states raises."""
        with Manager():
            A = Counter(num_states=5, name='A')
            with pytest.raises(ValueError, match="exceeds parent.num_states"):
                sample(A, num_states=10, with_replacement=False)
    ...
    def test_sample_iteration(self):
        """Iterate sampled counter and check states."""
        with Manager():
            A = Counter(num_states=10, name='A')
            sampled = [5, 2, 8]
            B = sample(A, sampled_states=sampled)
    ...
    def test_sample_with_name(self):
        """Sample counter can have a name."""
        with Manager():
            A = Counter(num_states=10, name='A')
            B = sample(A, num_states=3, seed=42, name='Sampled')
    ...
    def test_cannot_specify_both_num_states_and_sampled_states(self):
        ...
        with Manager():
            ...
            with pytest.raises(ValueError, match="Cannot specify both"):
                sample(A, num_states=5, sampled_states=[1, 2, 3])
...
class TestSampleOp:
    ...
    def test_sample_op_compute_num_states(self):
        ...
        op = SampleOp(num_parent_states=10, sampled_states=[1, 3, 5])
        assert op.compute_num_states((10,)) == 3
    
    def test_sample_op_decompose(self):
        """decompose maps to sampled index."""
        op = SampleOp(num_parent_states=10, sampled_states=[7, 2, 9])
    ...
    def test_sample_op_inactive(self):
        ...
        op = SampleOp(num_parent_states=10, sampled_states=[1, 2, 3])
        assert op.decompose(None, (10,)) == (None,)
    
    def test_sample_op_seed_stored(self):
        """Seed is stored for reference."""
        op = SampleOp(num_parent_states=10, num_states=5, seed=123)
        assert op.seed == 123
    ...
    def test_sample_op_no_seed_generates_random(self):
        ...
        op = SampleOp(num_parent_states=10, num_states=5)
...
class TestSampleComposition:
    ...
    def test_sample_of_product(self):
        ...
        with Manager():
            ...
            D = sample(C, num_states=5, seed=42)
            
            assert D.num_states == 5
...
class TestSampleFunction:
    ...
    def test_sample_with_name(self):
        ...
        with Manager():
            A = Counter(num_states=10, name='A')
            B = sample(A, num_states=5, seed=42, name='Sampled')
            assert B.name == 'Sampled'
    
    def test_sample_not_counter_raises(self):
        """sample with non-Counter raises BeartypeCallHintParamViolation."""
        with pytest.raises(BeartypeCallHintParamViolation):
            sample("not a counter", num_states=5)
    
    def test_sample_no_seed_generates_random(self):
        """sample without seed uses random seed."""
        with Manager():
            A = Counter(num_states=10, name='A')
            B = sample(A, num_states=5)  # No seed provided
