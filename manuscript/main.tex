...
To generate a sequence, PoolParty passes a \emph{state} --- an integer index that, together with a random seed, uniquely determines the output --- to the root. Generation then proceeds in two passes (Fig.~1B). In the \emph{backward pass}, PoolParty propagates the state from the root Pool back through the DAG. Each Operation decomposes the state it receives into an internal state and one or more states that are then passed to its input Pools. Each of the input Pools then passes its assigned state to its upstream Operation. In the \emph{forward pass}, each Operation generates a sequence based on its input sequences (if any) and its internal state. This process starts at the source Operations and proceeds forward until the root Pool receives the completed sequence. When generateing a library, PoolParty iterates the state passed to the root Pool sequentially, starting from zero and stopping at the number of sequences to be generated. 


\subsection{Sequential mode, random mode, and fixed mode}

The way each Operation interprets its state depends on its mode, which can be fixed, sequential, or random. The number of possible internal states of the operation is represented by a parameter \texttt{num\_states}. When \texttt{num\_states} is not \texttt{None}, the Operation's internal state will iterate sequentially from 0 through $\texttt{num\_states}$. 

\begin{itemize}
\item In \emph{sequential} mode, a finite number of distinct operations are performed. For example, when the \texttt{mutagenize} Operation is run in sequential model with \texttt{num\_mutations=1}, it will yield an output Pool representing all single-nucleotide mutations to each of the sequences of the input Pool. Which specific mutation is applied to each sequence depends on the value of the Operation's internal state. 

\item In \emph{random} mode, there is a stochastic aspect to the sequence that is returned. One example is the \texttt{shuffle} Operation, which randomly reorders the bases in a sequence. Users can choose to set the value of \texttt{num\_state} to some specific integer, in which case the Operation will cycle through only that many distinct ways of permuting sequence positions. Alternatively, one can set \texttt{num\_states=None}, in which every permutation will be chosen randomly. Users can ensure reproducibility when generating libraries by passing a random seed.

\item In \emph{fixed} mode, \texttt{num\_states=None} and the ouput sequence is uniquely determined by the input sequences. One example of a fixed Operaiton is \texttt{join}, which simply concatenates its input sequences together but does not alter them otherwise. 

\end{itemize}

\subsection{StateTracker}

The way each Operation decomposes the state it receives during the backward pass into an internal state and a set of Pool-specific states is designed to reverse the way the Operation combines its internal and input states during the forward pass. For many Operations, the state of the output sequence represents a Cartesian product between the Operation's internal state and the states of the input sequences. For example, the state of the sequence yielded by \texttt{join} represents the Cartesian product of each of the input sequences that are joined together. In such cases, mixed-radix decomposition is used in the backward pass. Some Operations deviate from this pattern, however. Of particular importance is the \texttt{stack} Operation, which yields a Pool representing a disjoint union among the input Pool, and which thus requires a different way of decomposint its state. 

Keeping track of how to compose and deompose states during the forward and backward passes presents a challenge in complex libraries. To address this challenge we developed a back-end package called StateTracker. [CONTINUE HERE]
