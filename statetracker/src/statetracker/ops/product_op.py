...
from ..imports import beartype, Sequence, Optional, math, Integral, State_type, Literal
...
def set_product_order_mode(mode: Literal['first_state_fastest', 'first_state_slowest']) -> None:
    ...
    if mode not in ('first_state_fastest', 'first_state_slowest'):
        raise ValueError(f"mode must be 'first_state_fastest' or 'first_state_slowest', got {mode!r}")
...
def product(states:Sequence[State_type], name:Optional[str]=None):
    ...
    if len(states) != len(set(states)):
        raise ValueError(f"product() does not allow duplicate states")
    if len(states) == 0:
